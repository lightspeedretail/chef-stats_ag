...
node['stats_ag']['custom_scripts'].each do |name,src_action|
  metrics_list.push(name.split('.', 1)[0])
  cookbook_file File.join(node['stats_ag']['scripts_dir'], name) do
    source name
    owner 'root'
    group 'root'
    mode '0755'
    cookbook node['stats_ag']['custom_scripts_cookbook']
    action src_action == "create" ? 'create' : 'remove'
  end
end
